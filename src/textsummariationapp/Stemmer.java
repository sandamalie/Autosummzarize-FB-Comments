...
public class Stemmer {
  ...
  private static HashMap m_l11 = null; //HashMap <Key, Value(Condition code)> or HashMap<K, V>. HashMap implements Map interface,It allows null key and null values 
  ...
  static {
    ...
    m_l11 = new HashMap(); //If here matched with the senetence then it needs to be remove
    m_l11.put("alistically", "B"); //The Lovins stemmer has 294 endings,29 conditions( A/B...) and 35 transformation rules, Each ending is followed by its condition code(<Stemming word> , <Condition code>) - Grouped by length, from 11 characters
    m_l11.put("arizability", "A"); // Based on Lovins stemming algorithm - 
    m_l11.put("izationally", "B"); //Here it is grouped by length, from 11 characters down to 1
    m_l10 = new HashMap();
    m_l10.put("antialness", "A"); // Here it is grouped by length, from 10 characters
    ...
    m_l9.put("ationally", "B"); // Here it is associated condition B and the ‘minimum stem length = '3'
  ...
  private String removeEnding(String word) {
	...
	if (conditionCode != null) { // 29 conditions
  ...
  private String recodeEnding(String word) {
    ...
    int lastPos = word.length() - 1; // Here include 294 endings, 29 conditions and 35 transformation rules
